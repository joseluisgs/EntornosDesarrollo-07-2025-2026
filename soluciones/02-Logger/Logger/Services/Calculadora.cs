...
public class Calculadora
{
    ...
    public decimal Dividir(decimal dividendo, decimal divisor)
    {
        ...
        _logger.Information("División completada: {Dividendo} / {Divisor} = {Resultado}", dividendo, divisor, resultado);
        return resultado;
    }
    ...
    public int Potencia(int baseNum, int exponente)
    {
        ...
        if (exponente < 0)
        {
            _logger.Error("Intento de potencia con exponente negativo. Base: {Base}, Exponente: {Exponente}", baseNum, exponente);
    ...
    public int Factorial(int numero)
    {
        ...
        if (numero > 20)
        {
            _logger.Warning("Factorial de número grande: {Numero}. Posible overflow.", numero);
        }
